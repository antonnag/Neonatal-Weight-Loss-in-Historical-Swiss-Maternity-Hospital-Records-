...
has_required_model_variation <- function(data, outcome) {
  ...
  test_data <- data %>% filter(!is.na(.data[[outcome]]), !is.na(feeding_cat))
...
run_stratified_breastfeeding_logistic <- function(data, strat_var, analysis_label) {
  ...
  out %>% mutate(excel_note = "For Mathilde/Noemie sheet: enter OR and upper CI for breastfeeding vs artificial. Missing values indicate insufficient variation/cases in that stratum.")
}
