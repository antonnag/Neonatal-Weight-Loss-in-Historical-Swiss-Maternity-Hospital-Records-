...
table_non_evaluable_weight_trajectory <- df_non_evaluable_weight_trajectory %>%
  ...
  mutate(
    percent = round(n / n_eligible * 100, 2)
  )
...
df_analysis_clean <- df_analysis_clean %>%
  mutate(
    ...
    feeding_cat = factor(
      feeding_cat,
      levels = c("artificial", "breastfeeding", "mixed", "other")
    )
  )
...
# 4) Core covariates
# ------------------------------------------------------------
# Use corrected gestational age where available; otherwise fall back to the
# original gestational-age variable.
...
# ------------------------------------------------------------
# Historical exposure is defined by overlap between the pregnancy interval and
# the exposure window. month_1 is the first month of conception / pregnancy
# start and birthdate is the delivery date. This avoids assigning exposure only
# by birth year, because a pregnancy can overlap WWI, the influenza pandemic, or
# both.
...
# Helper rule for interval overlap:
# pregnancy_start < exposure_end AND birthdate >= exposure_start.
# Note: month_1 is used as the pregnancy start variable, as recommended.
df_analysis_clean <- df_analysis_clean %>%
  mutate(
    ...
    # Pregnancy started after WWI ended.
    ...
    # Descriptive label only. Do not use this mutually exclusive variable for
    # inferential period adjustment, because WWI and pandemic exposure can
    # overlap. Primary period-adjusted models should use:
    #   pregn_during_WW1 + pregn_during_pandemic
    ...
    historical_period = factor(
      ...
      levels = c(
        "pre-war",
        "world_war_i_pregnancy",
        "influenza_pandemic_pregnancy",
        "post-war_or_post-pandemic"
      )
    )
  )

# Diagnostic tables to verify the period variables before modelling.
...
# Primary exposure-variable counts used for regression models.
...
# ------------------------------------------------------------
# Broad Grippe variable may include influenza before
# pregnancy, during pregnancy, at admission, or after delivery. Therefore, the
# analysis-specific variables below distinguish pregnancy-relevant influenza.
#
# Expected Grippe_cat coding:
# A = before pregnancy
# B = during pregnancy
# C = at admission to the maternity
# D = after delivery
#
# Primary direct exposure: B or C.
# Extended exposure: B, C, or D if the analysis wants to include peripartum/
# immediately postpartum recorded influenza as potentially pregnancy-related.
...
# ------------------------------------------------------------
# These are used in stratified breastfeeding subgroup analyses and Cochran-Q
# heterogeneity checks. PTB/LBW variables are standardized to readable factor labels.
...
# ------------------------------------------------------------
# To compare infants included in the analytical
# cohort with eligible live-born infants excluded because of non-evaluable NWL
# trajectories (<0% or >20%), the excluded dataset needs the same derived
# covariates as df_analysis_clean. Stillbirths are not included here because
# they are excluded by design before neonatal weight loss can be meaningfully
# defined.
