...
# Table 2. Neonatal weight loss according to feeding type
# Placement: Results 3.2 Distribution of neonatal weight loss
...
# Table 3. Pregnancy-based historical exposure variables and neonatal weight loss
# Placement: Results 3.3 Historical variation in neonatal weight loss
#
# Following Mathilde's recommendation, WWI and influenza pandemic exposure are
# treated as separate pregnancy-overlap variables. They are not forced into one
# mutually exclusive period factor, because some pregnancies can overlap both
# exposure windows.
...
# Characteristics of included infants and infants excluded because
# of non-evaluable neonatal weight trajectories
# ============================================================
# Feedback Mathilde/Katarina: This table checks whether exclusions based on
# NWL plausibility criteria (<0% or >20%) appear selective. The comparison is
# restricted to eligible live-born infants with birthweight and at least one
# postnatal weight measurement. Stillbirths are not included because they were
# excluded by design before NWL could be meaningfully defined.
...
df_exclusion_comparison <- bind_rows(
  df_analysis_clean %>%
    mutate(exclusion_group = "Included analytical cohort"),
  df_non_evaluable_weight_trajectory %>%
    mutate(exclusion_group = "Excluded non-evaluable NWL trajectory")
) %>%
  mutate(
    exclusion_group = factor(
      exclusion_group,
      levels = c(
        "Included analytical cohort",
        "Excluded non-evaluable NWL trajectory"
      )
    )
  )
...
safe_t_test_s7 <- function(var) {
  ...
  test_data <- df_exclusion_comparison %>%
    filter(!is.na(.data[[var]]), !is.na(exclusion_group))
  if (n_distinct(test_data$exclusion_group) < 2 || nrow(test_data) == 0) return(NA_real_)
  tryCatch(
    t.test(as.formula(paste(var, "~ exclusion_group")), data = test_data)$p.value,
...
safe_cat_test_s7 <- function(var) {
  ...
  test_data <- df_exclusion_comparison %>%
    filter(!is.na(.data[[var]]), !is.na(exclusion_group))
  if (n_distinct(test_data$exclusion_group) < 2 || n_distinct(test_data[[var]]) < 2) {
    return(NA_real_)
  }
